...
**Методы**
 ...
 - `protected onSubmit(evt: Event): void` : Обработчик события отправки формы. Предотвращает стандартное поведение браузера. Если форма валидна, генерирует событие с данными формы. Если невалидна, выводит предупреждение в консоль.
...
- `protected checkAddressValidity()`: Проверяет валидность адреса. Если адрес не валиден, устанавливает соответствующее сообщение об ошибке.
- `protected checkPaymentValidity()`: Проверяет, выбран ли способ оплаты, устанавливая соответствующее сообщение об ошибке.
- `protected onSubmit = (evt: Event)`: Обработчик отправки формы. Предотвращает стандартное поведение браузера. Генерирует событие 'ui:orderForm:submit' с данными адреса и способа оплаты. 
...
Класс для модального окна, расширяющий класс Component. Содержит поля для элементов, кнопки закрытия, а также создаёт слушатели нажатия кнопки закрытия для закрытия модального окна. Конструктор вызывает родительский класс.
...
1. Пользователь нажимает на карточку в каталоге товаров:
Это действие вызывает событие ui:cardCatalog:clicked.
2. Событие ui:cardCatalog:clicked вызывает обработчик, который обновляет данные в модели productData:
productData.selected устанавливается в качестве выбранного товара.
3. Пользователь нажимает на кнопку "Добавить" в карточке товара:
